...
def crm_vm():
    ...
    try:
        ...
        te=1
    mycheck(te == 1, f'{te}Cannot delete an active server')
...
def crm_nvms():
    ...
    for p in ps:
        try:
            print(p.sc.name,p.nvms,len(p.get_vms("any")))
        except Exception as e:
...
# fail a server.
    ...
    for p in ps:
        try:
            print(p.sc.name,p.nvms,p.state,len(p.get_vms("any")))
        except Exception as e:
            s=f'Delete_vm returned exception for {v.vc}'
            print(e)
...
# delete one of this idle vms.
    ...
    idlevms=crm.get_vms('idle')
    v1=idlevms[0]
    print(f'deleting {v1.vc.name}, {v1.sr.sc.name}')
...
#        print(v.vc.name)
    ...
    for v in vms:
        try:
            s = v.pvm.sr   
            t = isinstance(crm.mvd[v.vc.name],myvm)                       
            crm.delete_vm(v.vc.name)
            n = len(s.get_vms("any"))
            print(s.nvms)
            #print(v.pvm.state, v.pvm.vc.name, "was attached to" , s.sc.name, "Deleted VM attached:", n, "nvms total:",s.nvms,s.state,t)
        except Exception as e:
            s=f'Delete_vm returned exception for {v.vc}'
            print(e)  
# check for zero vms
